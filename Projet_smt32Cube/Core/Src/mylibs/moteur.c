...
static void setPWMsDutyCycle(int alpha)
{
    ...
    if (alpha < alphaActuel)
    {
        ...
        while (alphaActuel > alpha)
        {
            ...
            __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL2, ALPHA_MAX - alphaActuel); 
            HAL_Delay(1); 
        }
    }
    else
...
{
    __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL1, ALPHA_0);
    __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL2, ALPHA_MAX - ALPHA_0);
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL1); 
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL2); 
    HAL_TIM_PWMN_Start(&htim1, TIM_CHANNEL1); 
    HAL_TIM_PWMN_Start(&htim1, TIM_CHANNEL2); 
}
...
{
    __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL1, ALPHA_MIN); 
    __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL2, ALPHA_MIN); 
    HAL_TIM_PWM_Stop(&htim1, TIM_CHANNEL1);
    HAL_TIM_PWM_Stop(&htim1, TIM_CHANNEL2); 
    HAL_TIM_PWMN_Stop(&htim1, TIM_CHANNEL1); 
    HAL_TIM_PWMN_Stop(&htim1, TIM_CHANNEL2); 
}
